...
test('activity reads only the selected owner/profile and paginates without dropping rows',async()=>{
 ...
 const result=await activity('token',2,'library',2,transport);assert.equal(result.items.length,100);assert.equal(result.hasMore,true);
});
test('invalid categories, pages and inaccessible profiles cannot query history',async()=>{
 ...
 for(const args of [[2,'unknown',1],[2,'watched',0],[2,'progress',1],[NaN,'library',1]])await assert.rejects(activity('token',...args,transport));
});
